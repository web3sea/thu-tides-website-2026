...
export const WithPlaceholder = () => {
  ...
  return (
    <Frame label="Deliverables (multi-select)">
      <Combobox items={deliverables} multiple defaultValue={[]}>
        <ComboboxChips ref={anchor} className="w-96">
          <ComboboxValue>
            {(value: string[]) => (
              <React.Fragment>
                {value.map((v) => <ComboboxChip key={v}>{v}</ComboboxChip>)}
                <ComboboxChipsInput placeholder={'Add deliverables'} />
              </React.Fragment>
            )}
          </ComboboxValue>
        </ComboboxChips>
...
export const Typing = () => {
  ...
  return (
    <Frame label="Deliverables (multi-select)">
      <Combobox items={deliverables} multiple defaultInputValue="dr" defaultValue={['Reels']} open modal={false}>
        <ComboboxChips ref={anchor} className="w-96">
          <ComboboxValue>
            {(value: string[]) => (
              <React.Fragment>
