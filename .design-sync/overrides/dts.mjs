// forked from design-sync lib/dts.mjs - H1/H2/H3 are real components in components/typography.tsx but the ALL-CAPS constant filter (^[A-Z][A-Z0-9_]+$) drops them; require 2+ letters before allowing the constant heuristic
...
export const isComponentName = (n) => !n.endsWith('Props') && !(/^[A-Z][A-Z0-9_]+$/.test(n) && !/^H[1-6]$/.test(n))
